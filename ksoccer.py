...
import numpy as np
class KSoccer:
    def __init__(self, model='yolo11n.pt', source='camera', path=None, ruta_carpeta=None, save = False, scale_factor = 1,depurar = False):
    ...
    def process_frame(self, frame):
        ...
        results = self.yolo_model(frame, classes=[0], conf=0.5,half=True)
        
        # Process ball detections
        ball_results = self.model_pelota(frame, classes=[32], conf=0.1,half=True,augment=True)
    ...
    def get_dominant_color(self, image):
        """
        Simplified and more efficient color detection using mean color
        """
        try:
            ...
            # Simply calculate the mean color
            dominant_color = np.mean(valid_pixels, axis=0)
    ...
    def assign_teams(self, player_detections):
        """
        Team assignment to established colors or perform new clusters if needed 
        """
